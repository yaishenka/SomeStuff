...
void find_f (Node* t,int level, int* arr, int cont){
    arr [level]+=1;
    if ( arr[level] > arr[cont]) {
        arr[cont]=arr[level];
    }
    if (t->L!=nptr) find_f (t->L,level+1,arr,cont);
    if (t->R!=nptr) find_f (t->R,level+1,arr,cont);
...
int main()
{
    ...
    find_f(naive, 0, arr,cont);
    a=arr[cont];
    for (int i(0);i<cont+1;++i) arr[i]=0;
    find_f(root, 0, arr,cont);
    b=arr[cont];
    cout << b - a << endl;
    return 0;
}
